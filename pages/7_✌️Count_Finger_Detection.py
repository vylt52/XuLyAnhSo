...
def process_frame(img, detector, classifier, labels, offset=20, imgSize=300):
    ...
    if hands:
        for hand in hands:
            ...
            if (
                0 <= y - offset < img.shape[0]
                and 0 <= x - offset < img.shape[1]
...
def main():
    ...
    offset = 5
    imgSize = 300
    labels = ["One", "Two", "Three", "Four", "Five", "Zero"]
    ...
    if start_button:
        while cap.isOpened():
            ...
            img_output = process_frame(
                img, detector, classifier, labels, offset=offset, imgSize=imgSize
            )
